...
description: "Qué es, cómo funciona cada pieza, cómo se evalúa, un ejemplo en producción en Azure y una comparación de Azure · Google Cloud · código abierto"
date: 2026-09-25
...
| Cifra clave | Valor | Fuente |
|---|---|---|
| Menos fallos de retrieval | −67% | Contexto + BM25 + reranker (Anthropic, 2024) |
| Alucinación en RAG legal comercial | 17–33% | Stanford, 2024 |
| Respuestas sin alucinación (mejor RAG industrial) | 63% | CRAG benchmark, Meta 2024 |
| Acuerdo juez LLM ↔ humano | \>80% | Zheng et al., 2023 |

## Cómo leer este documento

> Compilado el: 2026-09-23.

- **(preview)**: la función existe, pero el proveedor todavía la tiene en *preview*. Puedes probarla, pero el proveedor no la recomienda para producción.
- **†**: dato de conocimiento general, **no verificado** contra la documentación oficial el 2026-09-23. Todo lo demás se comprobó ese día contra la documentación oficial o los papers citados.
- Las siglas se explican la primera vez que aparecen, y todas están en el **Glosario** al final.
- **Referencias**: en el cuerpo, los trabajos se citan por su nombre (p. ej. *SPLADE*, *ColBERT*, *RAGAS*); la lista completa de autores, el año y el enlace están en las **Referencias** al final.

## Parte I · Conceptos

### 1. Qué es RAG, explicado con una biblioteca

**RAG** (*Retrieval-Augmented Generation*, generación aumentada por recuperación) significa que, antes de responder, un modelo de lenguaje busca información en tus documentos y responde con base en lo que encontró, citando la fuente.

¿Por qué hace falta? El modelo de lenguaje no conoce los documentos internos de tu empresa, su conocimiento tiene una fecha de corte y puede inventar cosas (lo que se llama "alucinar"). Con RAG, la respuesta se apoya en documentos concretos y verificables, y actualizar el conocimiento no exige reentrenar el modelo.
...
<figure class="diagram"><div class="diagram-scroll"><svg style="min-width:965px" viewBox="0 0 1440 320"  xmlns="http://www.w3.org/2000/svg" role="img"
      ...
      <desc id="rag-library-desc">Un pipeline de izquierda a derecha con siete etapas — documentos, chunks, índice, búsqueda, reranker, modelo y evaluación — asociado a la metáfora de una biblioteca: una biblioteca de documentos se corta en fichas, se cataloga, un bibliotecario busca en ella, un experto (el reranker) filtra, un modelo redacta y un profesor califica; el paso de generación se resalta como el traspaso focal.</desc>
      <defs>
      ...
      <text x="320" y="216" font-family="Meslo, Menlo, monospace" font-size="8" fill="#4d6fa8" text-anchor="middle">Corta documentos</text>
      ...
      <text x="520" y="216" font-family="Meslo, Menlo, monospace" font-size="8" fill="#4d6fa8" text-anchor="middle">Archiva cada ficha</text>
      ...
      <text x="920" y="216" font-family="Meslo, Menlo, monospace" font-size="8" fill="#4d6fa8" text-anchor="middle">Elige las</text>
      <text x="920" y="228" font-family="Meslo, Menlo, monospace" font-size="8" fill="#4d6fa8" text-anchor="middle">5 mejores</text>
      ...
      <text x="1120" y="216" font-family="Meslo, Menlo, monospace" font-size="8" fill="#4d6fa8" text-anchor="middle">Redacta respuesta</text>
      ...
      <text x="1320" y="216" font-family="Meslo, Menlo, monospace" font-size="8" fill="#4d6fa8" text-anchor="middle">Califica: ¿recuperó</text>
      <text x="1320" y="228" font-family="Meslo, Menlo, monospace" font-size="8" fill="#4d6fa8" text-anchor="middle">bien? ¿respondió bien?</text>
      ...
      <text x="152" y="301" font-family="Meslo, Menlo, monospace" font-size="8" fill="#4d6fa8">Etapas de retrieval (ingesta → búsqueda → rerank)</text>
      <rect x="470" y="292" width="12" height="12" rx="2" fill="rgba(254,219,0,0.14)" stroke="#fedb00" stroke-width="1.2"/>
      <text x="490" y="301" font-family="Meslo, Menlo, monospace" font-size="8" fill="#4d6fa8">Generación — traspaso focal</text>
      <line x1="720" y1="298" x2="744" y2="298" stroke="#003da5" stroke-width="1" marker-end="url(#arrow)"/>
      <text x="752" y="301" font-family="Meslo, Menlo, monospace" font-size="8" fill="#4d6fa8">Traspaso de datos entre etapas</text>
    </svg></div><figcaption>RAG como biblioteca: documentos → chunks → índice → búsqueda → reranker → modelo → evaluación</figcaption></figure>


La evolución del campo suele describirse en tres etapas (Gao et al., 2023/24):

| Etapa | Idea |
|----|----|
| **Naive RAG** | Indexar → recuperar los top k → pegarlos en el prompt |
| **Advanced RAG** | Mejorar **antes** de buscar (reescribir la pregunta, hacer mejor chunking) y **después** (reranking, compresión) |
...
<desc id="rag-circuitos-desc">Arquitectura de un RAG con tres circuitos: preparar los documentos (fuentes, parsing y chunking, embeddings, índice), responder cada pregunta (búsqueda híbrida, reranker, modelo con citas) y evaluar con un golden set y muestreo para retroalimentar mejoras.</desc>
<defs>
...
<text x="796" y="264" fill="#4f5d75" font-size="9" font-family="Meslo, Menlo, monospace" text-anchor="middle">respuesta o “no lo sé”</text>
...
Es el paso de mayor impacto y el que más se descuida. Si el PDF se convierte a "texto plano", se pierden los títulos, las tablas se desordenan y cada chunk pierde su contexto.
...
<figure class="diagram"><div class="diagram-scroll"><svg style="min-width:670px" viewBox="0 0 1000 340"  xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="rag-parsing-title rag-parsing-desc">
  <title id="rag-parsing-title">Parsing: texto plano frente a parsing que respeta la estructura</title>
  <desc id="rag-parsing-desc">Comparación de antes y después del parsing de documentos. A la izquierda, la extracción de texto plano aplana un PDF y pierde sus títulos y la estructura de sus tablas. Una flecha rotulada modelo de layout apunta al panel derecho, donde el parsing que respeta la estructura conserva la jerarquía de títulos como Markdown y preserva la tabla.</desc>
  <defs>
  ...
  <text x="500" y="170" font-family="Meslo, Menlo, monospace" font-size="9" fill="#4d6fa8" text-anchor="middle" letter-spacing="0.04em">modelo layout</text>
  ...
  <text x="60" y="100" font-family="Meslo, Menlo, monospace" font-size="8" fill="#003da5" letter-spacing="0.14em">❌ EXTRACCIÓN TEXTO PLANO</text>
  ...
  <text x="60" y="150" font-family="Meslo, Menlo, monospace" font-size="10" fill="#4d6fa8">POLÍTICA TELETRABAJO 3.</text>
  <text x="60" y="166" font-family="Meslo, Menlo, monospace" font-size="10" fill="#4d6fa8">Elegibilidad Rol Días País</text>
  ...
  <text x="60" y="240" font-family="Meslo, Menlo, monospace" font-size="9" fill="#003da5">• tabla revuelta en una línea</text>
  ...
  <text x="600" y="120" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif" font-size="12" font-weight="600" fill="#000000">Títulos y tabla conservados</text>
  ...
  <text x="600" y="204" font-family="Meslo, Menlo, monospace" font-size="10" fill="#000000">| Rol     | Días/sem. |</text>
...
</svg></div><figcaption>Parsing: la extracción de texto plano pierde la estructura; el parsing que respeta la estructura conserva títulos y tablas</figcaption></figure>


Evidencia:

- **ColPali** (ICLR 2025): sus autores "suelen encontrar que optimizar el pipeline de ingesta da mejoras mucho mayores que optimizar el modelo de embeddings". ColPali indexa directamente **imágenes de las páginas**, sin extraer texto ni hacer chunking, y obtuvo 81.3 nDCG@5 frente a ~65–67 de los pipelines de parsing en el benchmark ViDoRe.
- **Unstructured, FinanceBench (2024)**: hacer chunking por elementos del documento (títulos, tablas) logró 53.2% de exactitud frente a 48.2% con chunks fijos de 512 tokens, y con la mitad de chunks.
- Estudio en turco (2026): el chunking que tiene en cuenta el layout ayuda mucho más en documentos con tablas que en documentos solo de texto.

Herramientas: Document Layout skill (Azure), Document AI Layout Parser (Google), Docling (IBM, código abierto). Ver la comparación en la sección 22.

### 4. Chunking

**Qué es:** dividir cada documento en trozos pequeños (*chunks*) que se indexan por separado.
...
| **Semántico** | Divide donde cambia el significado entre oraciones, medido con embeddings | Medio |
| **Basado en LLM ("agéntico")** | Un modelo decide dónde dividir | Alto |
| **Proposiciones** | Un modelo reescribe el texto en hechos atómicos | Alto |

#### 4.2 Lo que dice la evidencia: el chunking semántico está sobrevalorado

- **Vectara (2024), "Is Semantic Chunking Worth the Computational Cost?"**: "los costos computacionales asociados al chunking semántico no se justifican con ganancias de rendimiento consistentes". En los 4 datasets de documentos reales ganó el chunking de tamaño fijo (p. ej. HotpotQA, F1@5: 90.59 fijo vs 87.37 semántico). El modelo de embeddings importó más que el chunking.
- **Chroma (2024)**: la estrategia de chunking cambia el recall hasta en un 9%. El chunker semántico por defecto quedó ligeramente por debajo del promedio (83.6% de recall), mientras que un chunker recursivo de 200 tokens sin solapamiento llegó a 88.1%, cerca del mejor (91.9%, que usa un modelo de lenguaje y es mucho más caro).
- **NVIDIA (junio 2025)**: el chunking **por página** dio la mejor exactitud promedio (0.648) y la menor varianza entre datasets.
- **Biomedicina (2026)**: el chunking semántico ganó +8.4 puntos de F1 en un dataset, pero en otros el chunking de tamaño fijo "sigue siendo competitivo o mejor". Depende del dominio.

#### 4.3 Lo que sí funciona: añadir contexto a cada chunk
...
- **Anthropic, Contextual Retrieval (sep 2024)**: un modelo genera 50–100 tokens de contexto que se anteponen a cada chunk. Tasa de fallos de retrieval (dentro de los 20 primeros resultados), partiendo de una línea base de 5.7%:
  - Contexto solo en los vectores: 3.7% (**−35%**)
  - Contexto en los vectores **y** en BM25: 2.9% (**−49%**)
  - Todo lo anterior **más un reranker**: 1.9% (**−67%**)
  - Costo único: ~\$1.02 por millón de tokens de documentos, usando prompt caching.
  - ⚠️ Los tres porcentajes son reducciones respecto de la línea base (5.7%). El paso atribuible al reranker es de 2.9% a 1.9%.
- **Late chunking (Jina, 2024)**: primero se calcula el embedding del documento completo y solo después se divide en chunks, así cada vector "sabe" de qué documento viene. Mejora nDCG@10 entre +2.7% y +3.6% sin reentrenar.

#### 4.4 Tamaños para empezar

- Preguntas factuales cortas: 64–128 tokens. Preguntas que necesitan contexto amplio: 512–1024 tokens (Bhat et al., 2025).
- Azure recomienda empezar con **512 tokens y 25% de solapamiento**, y añadir el título del documento a los chunks intermedios.
- **Regla de oro:** empieza con chunking recursivo o por sección/página de 256–512 tokens, no partas las tablas, añade contexto y **ajusta midiendo** (Parte VI).

### 5. Convertir texto en vectores (embeddings)
...
<figure class="diagram"><div class="diagram-scroll"><svg style="min-width:549px" viewBox="0 0 820 380"  xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="rag-embed-title rag-embed-desc">
  <title id="rag-embed-title">Los embeddings acercan los significados similares</title>
  <desc id="rag-embed-desc">Un espacio vectorial conceptual. Los puntos de laptop barata y notebook económica quedan juntos porque significan casi lo mismo, mientras que política de vacaciones queda lejos porque no tiene relación. La distancia en el espacio codifica diferencia de significado, no palabras compartidas.</desc>
  <defs>
  ...
  <text x="56" y="88" font-family="Meslo, Menlo, monospace" font-size="8" fill="#003da5" letter-spacing="0.14em">ESPACIO DE EMBEDDINGS · DISTANCIA = DIF. DE SIGNIFICADO</text>
  ...
  <text x="66" y="365" font-family="Meslo, Menlo, monospace" font-size="8" fill="#4d6fa8">Significado similar — vecinos cercanos</text>
...
</svg></div><figcaption>Los embeddings acercan los significados similares y alejan el texto sin relación</figcaption></figure>
...
| **Sparse aprendido** | Lista de términos (casi todos en cero) con pesos, ampliada con términos relacionados | SPLADE, ELSER (Elastic), modo sparse de BGE-M3 |
| **Multivector** | Un vector por token; se compara token a token | ColBERT, ColPali |

Consejos:

- **Mide con tus propios datos.** Los leaderboards públicos (como MTEB) cambian cada mes y no siempre reflejan tu dominio.
- **Fija la versión del modelo.** Si mezclas vectores de modelos distintos, las búsquedas dejan de tener sentido.

### 6. Índice y permisos

El **índice** guarda cada chunk con su texto, su vector y sus **metadatos**:

| Campo(s) del índice                | Propósito                          |
|------------------------------------|------------------------------------|
| `id, content, content_vector`      | el texto del chunk y su embedding  |
| `title, section, page, source_url` | de dónde viene                     |
| `allowed_groups` 🔒                | grupos autorizados a verlo         |
...
BM25 es el algoritmo clásico de búsqueda por palabras clave (léxica) y el que viene por defecto en Elasticsearch, OpenSearch y Azure AI Search. Funciona sobre un **índice invertido**, como el índice alfabético al final de un libro:
...
Su puntuación combina tres ingredientes:
...
| **Longitud del documento** | Un texto corto que contiene la palabra puntúa más que uno largo (parámetro `b`, 0.75 por defecto †) | Un párrafo específico le gana a un manual entero |

- ✅ Rápido, sin modelos, explicable, excelente para **términos exactos** (códigos, siglas, nombres propios).
- ❌ No entiende sinónimos: *"laptop barata"* no encontrará *"notebook económica"*.

### 8. Búsqueda semántica: por significado

#### 8.1 Vectores densos (búsqueda de vecinos más cercanos)

La pregunta se convierte en un vector y se recuperan los chunks más cercanos. Para hacerlo rápido sobre millones de vectores se usa un índice aproximado (normalmente **HNSW**, un grafo de vecinos).

- ✅ Entiende sinónimos, paráfrasis y distintos idiomas.
- ❌ Es una caja negra (no puedes explicar por qué algo coincidió), puede confundir códigos casi idénticos (ORA-00942 vs ORA-00943) y usa más memoria. Para reducir la memoria existe la **cuantización** (comprimir los números, por ejemplo a 8 bits o a binario).

#### 8.2 Sparse aprendido (SPLADE, ELSER)

Es un punto intermedio: un modelo **expande** el texto con términos relacionados y sus pesos, y luego busca en un índice invertido, como BM25.

```
"cheap laptop" → { laptop: 2.1, notebook: 1.8, computer: 1.2, cheap: 1.9, budget: 1.5, price: 0.9 }
```

- ✅ Más explicable que los vectores densos, y encuentra sinónimos.
- ❌ Depende del idioma del modelo (ELSER se recomienda solo para inglés) y tiene un límite de tokens (ELSER codifica los primeros 512 tokens de cada campo).

#### 8.3 Cuál gana

| Consulta | BM25 | Denso | Sparse aprendido |
|----|----|----|----|
| *"laptop barata"* → doc con *"notebook económica"* | ❌ | ✅ | ✅ |
| *"error ORA-00942"* → doc con ese código | ✅ | ⚠️ | ✅ |
| *"¿puedo trabajar desde casa?"* → doc con *"teletrabajo"* | ❌ | ✅ | ✅ (si el idioma está soportado) |
| Explicar por qué coincidió | ✅ | ❌ | 🟡 |
| Costo | 💚 mínimo | 🟠 modelo + memoria | 🟡 modelo |

Ninguno gana siempre; por eso se combinan.
...
<desc id="rrf-fusion-desc">La pregunta se busca en paralelo con BM25 y con vectores; RRF fusiona el top 50 de cada lista por posición, un reranker los reordena y los 5 mejores chunks llegan al modelo.</desc>
<defs>
...
<text x="928" y="64" fill="#2d3142" font-size="14" font-style="italic" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif" text-anchor="end">RRF usa solo el orden:</text>
...
<text x="496" y="372" fill="#2d3142" font-size="8" font-family="Meslo, Menlo, monospace">Fusión (foco)</text>
</svg></div><figcaption>Figura · Búsqueda híbrida: dos búsquedas, una fusión RRF y un reranker</figcaption></figure>


#### 9.1 El problema

Las dos búsquedas devuelven puntuaciones en **escalas incompatibles**:
...
Sumar 12.4 + 0.81 no tiene sentido; sería como sumar euros y kilos.

#### 9.2 La solución: RRF (Reciprocal Rank Fusion)

RRF ignora las puntuaciones y usa **solo la posición** de cada documento en cada lista:
...
| Doc   | Aporte BM25       | Aporte vector       | **Total**   | Final |
|-------|-------------------|---------------------|-------------|-------|
...
<figure class="diagram"><div class="diagram-scroll"><svg style="min-width:643px" viewBox="0 0 960 500" xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="rrf-scores-title rrf-scores-desc">
      <title id="rrf-scores-title">Ejemplo de RRF con k = 60: puntuación por documento</title>
      <desc id="rrf-scores-desc">Puntuación RRF final de cuatro documentos: doc_A 0.03252 y doc_B 0.03226 aparecen en ambas listas y superan claramente a doc_C 0.01613, solo vectorial, y a doc_D 0.01587, solo BM25.</desc>
      <rect width="100%" height="100%" fill="#ffffff"/>
      ...
      <text x="480" y="376" fill="#4f5d75" font-size="8" font-family="Meslo, Menlo, monospace" text-anchor="middle" letter-spacing="0.14em">SCORE RRF = Σ 1/(60 + POSICIÓN)</text>
      ...
      <text x="496" y="232" fill="#4f5d75" font-size="12" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif" font-style="italic">solo vector</text>
    ...
    </svg></div><figcaption>Figura · Ejemplo de RRF con k = 60: puntuación por documento</figcaption></figure>


doc_A gana porque está **alto en ambas listas**. RRF premia el **consenso** entre los dos métodos.
...
| **k = 60** | 0.0164 | 0.0161 | casi igual ✓ — lo que cuenta es estar bien en varias listas |

El valor viene del paper original (Cormack, Clarke y Büttcher, SIGIR 2009), donde se eligió de forma empírica. Azure AI Search documenta que funciona mejor con valores pequeños como 60.

#### 9.4 Ventajas, límites y variantes

- ✅ Sin calibrar escalas, nada que entrenar, y acepta N listas (BM25, varios vectores, varias reformulaciones de la pregunta).
- ⚠️ **Ignora la magnitud**: ser primero "por mucho" vale lo mismo que ser primero "por un pelo".
- ⚠️ **Una lista mala cuenta igual**: si la búsqueda vectorial devuelve basura, esa basura también recibe puntos.
- **RRF ponderado**: una de las listas pesa más (por ejemplo, la lista vectorial ×2). Disponible en Azure (*vector weighting*), en el `EnsembleRetriever` de LangChain † y en Google Vector Search con `rrf_ranking_alpha`.
- **Alternativa, combinación lineal**: normalizar las puntuaciones y sumarlas con pesos. Aprovecha la magnitud, pero hay que calibrarla con datos. Ejemplos: el retriever `linear` de Elasticsearch, o DBSF en Qdrant †.
- **RRF no es un reranker.** Solo fusiona listas. El reranker viene después.

### 10. Estrategias avanzadas de retrieval

| Técnica | Qué hace | Evidencia clave |
|----|----|----|
| **Reescritura con historial** | Convierte *"entonces, ¿cuántos días?"* en una pregunta completa usando el chat anterior | Práctica estándar |
...
| **Step-back** | Primero preguntar algo más general | +27% en TimeQA, +7% en MuSiQue (Google DeepMind) |
| **Descomposición** | Divide una pregunta compleja en subpreguntas | Base de la búsqueda agéntica |
| **RAPTOR / documento padre** | Resúmenes jerárquicos; se busca por chunk pequeño y se devuelve el grande | RAPTOR + GPT-4: +20% absoluto en QuALITY |
...
| **Adaptativo** (Self-RAG, Corrective RAG, Adaptive-RAG) | Decide cuándo buscar, cuánto, y corrige si la búsqueda salió mal | Adaptive-RAG enruta según la complejidad de la pregunta |
| **Agéntico / "Deep Research"** | Búsqueda iterativa entrenada con aprendizaje por refuerzo | Search-R1: +41% (7B) sobre el RAG base; OpenAI Deep Research tarda de 5 a 30 minutos por tarea |
...
Elasticsearch ofrece cuatro piezas "semánticas" distintas, que conviene no confundir:
...
<figure class="diagram"><div class="diagram-scroll"><svg style="min-width:670px" viewBox="0 0 1000 360"  xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="rag-es-title rag-es-desc">
  <title id="rag-es-title">Elasticsearch: tres enfoques de retrieval sobre un mismo tipo de campo</title>
  <desc id="rag-es-desc">Tres enfoques lado a lado en Elasticsearch — A vectores densos kNN, B sparse aprendido con ELSER y C reranking semántico — se apoyan en un tipo de campo compartido, D semantic_text, que divide el texto en chunks y genera embeddings automáticamente, y que alimenta los enfoques denso y de reranking semántico.</desc>
  <defs>
  ...
  <text x="500" y="122" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif" font-size="13" font-weight="600" fill="#000000" text-anchor="middle">Sparse aprendido</text>
  ...
  <text x="500" y="298" font-family="Meslo, Menlo, monospace" font-size="9" fill="#4d6fa8" text-anchor="middle">hace el chunking y genera embeddings automáticamente — el default de bajo esfuerzo que alimenta A y C</text>
  <!-- legend -->
  ...
  <text x="130" y="353" font-family="Meslo, Menlo, monospace" font-size="8" fill="#4d6fa8">Enfoque de retrieval</text>
  <rect x="300" y="344" width="12" height="12" rx="2" fill="rgba(254,219,0,0.14)" stroke="#fedb00" stroke-width="1.4"/>
  <text x="318" y="353" font-family="Meslo, Menlo, monospace" font-size="8" fill="#4d6fa8">Campo compartido que los alimenta</text>
</svg></div><figcaption>Elasticsearch: tres enfoques de retrieval (A·B·C) sobre el campo compartido semantic_text (D)</figcaption></figure>


- **A. Denso**: `dense_vector` + consulta `knn`. Cuantización int8, int4 y BBQ. Modelos disponibles: E5 (multilingüe), Jina (vía el Elastic Inference Service) o externos (OpenAI, Azure OpenAI, Cohere, Bedrock, Vertex AI, Hugging Face).
- **B. ELSER**: expande términos; no genera sinónimos sino asociaciones aprendidas. Según el propio benchmark BEIR de Elastic, mejora en promedio un **18% en nDCG@10 sobre BM25** (10 victorias, 1 empate, 1 derrota). Recomendado para inglés, lee 512 tokens por campo y requiere una suscripción de pago.
- **C. Reranker**: el retriever `text_similarity_reranker` o el comando `RERANK` en ES\|QL.
- **D. `semantic_text`** (GA desde la versión 9.0). ⚠️ Si no fijas `inference_id`, después de una actualización de versión los índices nuevos pueden usar otro modelo. **En producción, fija siempre el modelo.**
...
<figure class="diagram"><div class="diagram-scroll"><svg style="min-width:643px" viewBox="0 0 960 520" xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="elastic-retrievers-title elastic-retrievers-desc">
      ...
      <desc id="elastic-retrievers-desc">Tres niveles anidados: el retriever externo text_similarity_reranker (foco) reordena el top 50 con un modelo de rerank; dentro, rrf fusiona dos retrievers standard: match con BM25 sobre content y semantic sobre un campo semantic_text.</desc>
      <defs>
...
**Búsqueda híbrida con reranker, en una sola llamada:**
...
Otras formas de fusionar: el retriever `linear` (normalizadores `minmax` o `l2_norm`) y, en ES\|QL, `FORK` + `FUSE` (RRF o LINEAR) + `RERANK`. En el formato multicampo, Elastic normaliza los campos léxicos y semánticos para que cada grupo aporte el 50%.

⚠️ **Trampa de vocabulario:** en Elastic, *"semantic search"* significa buscar con embeddings. En Azure, *"semantic ranker"* es un **reranker**.
...
Un **reranker** toma los ~50–150 candidatos de la búsqueda y los reordena leyendo **la pregunta y cada chunk juntos**. Es más preciso que comparar vectores, pero más lento, así que solo se aplica a unos pocos candidatos.
...
<desc id="bi-vs-cross-encoder-desc">El bi-encoder convierte la pregunta y el chunk en vectores por separado y compara su distancia, rápido para millones de chunks; el cross-encoder lee pregunta y chunk juntos y da una puntuación de relevancia precisa para 50–150 candidatos.</desc>
<defs>
...
<text x="544" y="316" fill="#7a8399" font-size="8" font-family="Meslo, Menlo, monospace" text-anchor="middle" letter-spacing="0.06em">PUNTÚA</text>
...
<text x="360" y="124" fill="#2d3142" font-size="12" font-weight="600" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif" text-anchor="middle">Vector pregunta</text>
...
<text x="360" y="188" fill="#2d3142" font-size="12" font-weight="600" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif" text-anchor="middle">Vector chunk</text>
...
<text x="656" y="332" fill="#2d3142" font-size="12" font-weight="600" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif" text-anchor="middle">Score de relevancia</text>
...
**Arquitectura típica en dos etapas:** búsqueda híbrida (prioriza la cobertura) → reranker sobre 100–150 candidatos → pasar entre 10 y 20 chunks al modelo que redacta la respuesta.
...
| **Reranker con razonamiento** (2025–26) | Rank1, Rank-R1, ReasonRank | En BRIGHT (búsqueda que requiere razonar), el mejor modelo de MTEB cae de 59.0 a 18.3; razonar sobre la pregunta aporta hasta +12.2 |
...
| Semantic ranker | Microsoft (Azure AI Search) | Servicio gestionado | Reordena el top 50, puntuación de 0 a 4 |
| Ranking API | Google | Servicio gestionado | Hasta 1000 chunks por llamada, puntuación de 0 a 1 |

#### 12.4 Reglas prácticas

- Es la mejora **más barata y más probada** (Anthropic: la tasa de fallos baja de 2.9% a 1.9% solo con añadir un reranker).
- **El reranker no busca, solo reordena.** Si el documento correcto no está entre los candidatos, no puede salvarte. Por eso primero se mide la cobertura del retrieval (recall).
- Diferenciador actual: **rerankers que siguen instrucciones**, a los que se les pueden dar reglas de negocio como "prioriza el contenido reciente".
- Todos los proveedores dicen ser los mejores. Mide con tus propios datos.
- Complementos útiles:
  - **MMR**: elimina chunks redundantes.
  - **Compresión**: LongLLMLingua da +21.4% de calidad con ~4× menos tokens.
  - **Ubicación en el prompt**: lo más relevante al principio o al final ("Lost in the Middle").
...
2.  **Durante**: si el reranker no deja ningún chunk por encima del umbral, responder "No encontré esa información" en lugar de inventar.
3.  **Después**: comprobar que cada oración de la respuesta esté respaldada por los chunks. Si falla, regenerar o responder con cautela.

## Parte VI · Evaluación

### 14. La idea clave: dos pruebas separadas
...
<figure class="diagram"><div class="diagram-scroll"><svg style="min-width:643px" viewBox="0 0 960 580" xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="eval-cuadrante-title eval-cuadrante-desc">
      ...
      <desc id="eval-cuadrante-desc">Matriz 2×2 que cruza si el retrieval encontró los documentos correctos con si la respuesta fue buena, e indica la acción para cada caso; la prioridad es arreglar primero el retrieval cuando ambos fallan.</desc>
      <defs>
      ...
      <text x="176" y="120" fill="#4f5d75" font-size="8" font-weight="500" font-family="Meslo, Menlo, monospace" letter-spacing="0.18em">01 · RETRIEVAL NO / RESPUESTA SÍ</text>
      ...
      <text x="536" y="120" fill="#4f5d75" font-size="8" font-weight="500" font-family="Meslo, Menlo, monospace" letter-spacing="0.18em">02 · RETRIEVAL SÍ / RESPUESTA SÍ</text>
      ...
      <text x="536" y="200" fill="#4f5d75" font-size="12" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif">Mantener y monitorear.</text>
      <rect x="160" y="296" width="280" height="160" rx="6" fill="rgba(235,108,54,0.14)" stroke="#eb6c36" stroke-width="1.2"/>
      <text x="176" y="320" fill="#4f5d75" font-size="8" font-weight="600" font-family="Meslo, Menlo, monospace" letter-spacing="0.18em">03 · RETRIEVAL NO / RESPUESTA NO</text>
      <text x="176" y="352" fill="#2d3142" font-size="16" font-weight="600" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif">Arreglar retrieval</text>
      <text x="176" y="372" fill="#2d3142" font-size="16" font-weight="600" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif">primero</text>
      ...
      <text x="536" y="320" fill="#4f5d75" font-size="8" font-weight="500" font-family="Meslo, Menlo, monospace" letter-spacing="0.18em">04 · RETRIEVAL SÍ / RESPUESTA NO</text>
      <text x="536" y="352" fill="#2d3142" font-size="16" font-weight="600" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif">Arreglar el prompt</text>
      ...
      <text x="832" y="504" fill="#4f5d75" font-size="12" font-weight="600" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif" text-anchor="end">Retrieval: ¿encontró los documentos correctos?</text>
...
Si solo miras la respuesta final, no sabes qué arreglar. Microsoft llama *process evaluation* a evaluar el paso de retrieval y *system evaluation* a evaluar la respuesta.

### 15. El golden dataset (la "hoja de respuestas")

Es un conjunto de 100 a 300 preguntas, cada una con su respuesta correcta y los documentos que deberían aparecer:
...
| Expertos del negocio (RR. HH., legal) | Casos difíciles y trampas |
| Generación sintética (RAGAS, simuladores de los proveedores cloud) | Cobertura rápida, **siempre revisada por una persona** |
| **Preguntas sin respuesta** en los documentos | Comprueban que el sistema diga "no lo sé" en lugar de inventar |

### 16. Métricas de retrieval, con números

**Ejemplo:** para una pregunta, los documentos correctos son **A y C**. El buscador devolvió `[B, A, D, C, E]`.
...
| **MRR** (posición del primer acierto) | ¿Qué tan arriba está el primer correcto? | 1/2 | **0.50** |
| **nDCG@5** (calidad del orden) | ¿Están los correctos lo más arriba posible? | Real = 1/log₂3 + 1/log₂5 = 1.06; ideal = 1 + 1/log₂3 = 1.63 | **0.65** |

**Cómo leerlo:**

- **Recall@50 bajo** → el problema está en el retrieval (chunking, embeddings, falta BM25). El reranker no lo va a arreglar.
- **Recall@50 alto pero nDCG@5 bajo** → el problema está en el reranker.

### 17. Métricas de respuesta, con números

Respuesta del sistema: *"Tienes 2 días por semana \[1\], 3 a partir de enero de 2026 \[2\], y puedes elegir los viernes."*

| Afirmación de la respuesta | ¿Respaldada por los chunks? |
|----------------------------|-----------------------------|
| `"2 days/week"`            | ✓                           |
| `"3 from January 2026"`    | ✓                           |
...
Microsoft lo plantea de la misma forma: la fidelidad al contexto es el lado de la precisión (no añadir nada) y la completitud es el lado de la cobertura (no dejar fuera nada crítico).

**Evaluación por "nuggets"** (TREC 2024): se definen los hechos atómicos que debe contener una buena respuesta y se cuenta cuántos aparecen.

### 18. El LLM como juez

Nadie revisa 10,000 respuestas a mano, así que otro modelo hace de profesor. Pero tiene sesgos:

- prefiere la primera opción que ve (posición);
- prefiere las respuestas largas (verbosidad);
- prefiere el texto de su propia familia de modelos (autopreferencia).

**Receta:**

1.  Una persona etiqueta 50–100 casos.
2.  Medir el acuerdo juez–humano (kappa de Cohen, exactitud, F1).
3.  El juez debe ser de una **familia de modelos distinta** a la del generador.
4.  El juez debe explicar su puntuación.
5.  Puntuar afirmación por afirmación (RAGChecker) o por nuggets es mejor que dar una sola puntuación global.

**Evidencia:**

- GPT-4 como juez alcanza más de un 80% de acuerdo con humanos, el mismo nivel que entre dos humanos (Zheng et al., 2023).
- En TREC 2024, el acuerdo perfecto humano–GPT-4o fue del 56%, y del 72% cuando la persona corregía la etiqueta del modelo.
- Conclusión: el juez LLM es confiable para comparar sistemas y menos confiable pregunta por pregunta.
- **ARES** combina unos cientos de etiquetas humanas con el juez automático para producir **intervalos de confianza** estadísticamente válidos.
...
<figure class="diagram"><div class="diagram-scroll"><svg style="min-width:643px" viewBox="0 0 960 664" xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="eval-bucle-title eval-bucle-desc">
      ...
      <desc id="eval-bucle-desc">Ciclo de seis pasos en sentido horario: golden dataset, evaluación offline, quality gate (foco), despliegue, evaluación continua y fallos reales con votos negativos, que retroalimentan el golden dataset; en el centro, las métricas de retrieval y de respuesta.</desc>
      <defs>
      ...
      <text x="480" y="320" fill="#2d3142" font-size="12" font-weight="600" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif" text-anchor="middle">Métricas: retrieval +</text>
      <text x="480" y="340" fill="#2d3142" font-size="12" font-weight="600" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif" text-anchor="middle">respuesta</text>
      ...
      <text x="688" y="452" fill="#4f5d75" font-size="9" font-weight="400" font-family="Meslo, Menlo, monospace" text-anchor="middle">bloquea regresiones</text>
      ...
      <text x="272" y="192" fill="#2d3142" font-size="12" font-weight="600" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif" text-anchor="middle">Fallos reales</text>
...
**Ejemplo de quality gate** en integración continua: Recall@10 no baja más de 2 puntos · faithfulness ≥ 95% · "no lo sé" correcto ≥ 90%. En producción se muestrea un porcentaje del tráfico real y se evalúa sin respuesta de referencia (faithfulness, relevancia de la respuesta, relevancia del contexto), junto con 👍/👎, latencia y costo.

Los umbrales son **ejemplos**; los fija el negocio.

**Barrido de parámetros.** Microsoft recomienda probar combinaciones y medir cuál gana. Los siguientes números son **ilustrativos**, no resultados reales:

| Configuración | Recall@10 | nDCG@5 | Faithfulness | Latencia |
|----|----|----|----|----|
| solo vectorial | 0.71 | 0.58 | 0.90 | 0.8 s |
| híbrida | 0.84 | 0.66 | 0.92 | 0.9 s |
| **híbrida + rerank** ← elegida | 0.84 | **0.79** | 0.95 | 1.3 s |
| \+ agéntica <span style="font-weight:400">(solo preguntas complejas)</span> | 0.88 | 0.81 | 0.95 | 3.5 s |

**Reglas:**

- No más de unas 5 métricas por aplicación (DeepEval).
- Usa los mismos evaluadores en desarrollo y en producción (MLflow / Databricks).
- En producción solo puedes usar métricas que no necesiten una respuesta correcta.

### 20. Por qué importa: la alucinación en sistemas reales

| Estudio | Resultado |
|----|----|
| Stanford (2024): herramientas legales comerciales con RAG | Alucinan entre el **17% y el 33%** de las veces |
...
| FinanceBench (2023) | GPT-4-Turbo con retrieval falló o se negó a responder en el 81% de los casos |
...
| FaithBench (2024) | Los mejores detectores de alucinaciones rondan el **50%** de exactitud en casos difíciles |

Varias de estas cifras son de 2023–2024, con modelos más antiguos. Cítalas siempre con año y modelo.

## Parte VII · Un ejemplo en producción en Azure

### 21. Copiloto de políticas internas, paso a paso

**Caso:** una empresa de 20,000 empleados. Documentos de RR. HH., legal y compras en SharePoint y Blob Storage. Requisitos: permisos por usuario, citas en cada respuesta y "no lo sé" cuando no hay información.
...
<desc id="azure-arquitectura-desc">Arquitectura en Azure de un copiloto de políticas: la ingesta lleva los documentos de SharePoint o Blob, mediante un indexer con un skillset, a Azure AI Search; la app responde al usuario con búsqueda, Azure OpenAI y Content Safety; Application Insights y Foundry evalúan y observan.</desc>
<defs>
...
**Equivalencias con un stack LangChain + Chroma/Qdrant:**

| Stack de código abierto | En Azure |
|----|----|
| Loaders de LangChain + text splitter | Indexer + Document Layout skill + chunking |
...
| Tu propia reescritura de consultas | Query rewriting del semantic ranker (preview) o agentic retrieval |
| GraphRAG | Microsoft GraphRAG como índice aparte, solo para preguntas globales |

#### 21.2 Una pregunta, de punta a punta

Ana, gerente en Madrid, preguntó antes por su contrato. Ahora escribe: **"entonces, ¿cuántos días puedo teletrabajar?"**
...
<figure class="diagram"><div class="diagram-scroll"><svg style="min-width:643px" viewBox="0 0 960 736" xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="azure-secuencia-title azure-secuencia-desc">
      ...
      <desc id="azure-secuencia-desc">Secuencia en la que la app reescribe la pregunta de Ana con Azure OpenAI, recupera chunks con búsqueda híbrida y el semantic ranker en Azure AI Search, genera una respuesta con citas y la verifica con Content Safety antes de responder con citas o “no lo sé”.</desc>
      <defs>
      ...
      <text x="388" y="268" fill="#7a8399" font-size="8" font-family="Meslo, Menlo, monospace" text-anchor="middle">búsqueda híbrida + filtro de permisos</text>
      ...
      <text x="756" y="584" fill="#7a8399" font-size="8" font-family="Meslo, Menlo, monospace" text-anchor="middle">respaldado / no respaldado</text>
      ...
      <text x="204" y="632" fill="#7a8399" font-size="8" font-family="Meslo, Menlo, monospace" text-anchor="middle">respuesta citada o «no lo sé»</text>
...
1.  **Reescritura:** el modelo usa el historial del chat y genera la pregunta *"días de teletrabajo permitidos para un gerente en España"*.

2.  **Búsqueda híbrida:** BM25 y la búsqueda vectorial corren en paralelo y se fusionan con RRF (k=60). Antes de puntuar, el **filtro de seguridad** elimina todo lo que Ana no tiene permiso para ver.

3.  **Semantic ranker:** toma solo el **top 50** y lo puntúa de 0 a 4:
    ...
    Los chunks con puntuación \< 2 se descartan. Si no queda ninguno, la respuesta es "No encontré esa información". Microsoft advierte que la distribución de puntuaciones puede variar ligeramente, así que los umbrales no deberían ser demasiado finos.

4.  **Generación con citas** usando la estructura de prompt de la sección 13.

5.  **Verificación de fidelidad:** se comprueba que cada oración esté respaldada por los chunks. Si falla, la respuesta se regenera o se da con cautela.

**Si la pregunta es compleja** (*"compara el teletrabajo en España vs México y dime cuál aplica si me traslado"*): el **agentic retrieval** de Azure AI Search divide la pregunta en subconsultas, las ejecuta en paralelo, reordena cada una con el semantic ranker y combina los resultados. La planificación de consultas basada en LLM y la síntesis de respuestas están en **preview**.

**Si la pregunta es global** (*"¿qué temas se repiten en todas las políticas de 2026?"*): ahí es donde GraphRAG vale la pena.

#### 21.3 Evaluación en Azure (Microsoft Foundry)

| Evaluador | Tipo | Necesita respuesta correcta | Estado |
|----|----|----|----|
| **Document Retrieval** | Retrieval: NDCG, XDCG, Fidelity, Max Relevance, Holes | Sí (etiquetas de relevancia) | GA |
| **Retrieval** | Retrieval, juzgado por un modelo de lenguaje (escala 1–5) | No | GA |
...
- Escala de 1 a 5; aprueba con 3 por defecto.
- **Evaluación continua**: evalúa muestras del tráfico real (porcentaje configurable, hasta 1000 solicitudes por hora) y envía los resultados a Application Insights, vinculados a las trazas.

## Parte VIII · Comparación: Azure vs Google Cloud vs código abierto

### 22. Azure vs Google Cloud vs código abierto

**Nombres nuevos** (verificados el 2026-09-23):
...
| **3. Chunking** | Chunks con contexto | Document Layout skill (por sección, o tamaño fijo con solapamiento) y Text Split skill | El Layout Parser hace chunking por estructura y añade los títulos padre. RAG Engine permite fijar tamaño y solapamiento | Text splitters de LangChain y LlamaIndex; chunking de Docling † |
| **4. Embeddings** | Texto → números | Azure OpenAI **text-embedding-3-large / -small** | **gemini-embedding-001** (hasta 3072 dimensiones, 2048 tokens por texto), text-embedding-005 (inglés y código), text-multilingual-embedding-002 | **BGE-M3** (denso + sparse + multivector, más de 100 idiomas), **Qwen3-Embedding** (Apache 2.0), multilingual-E5 |
| **5. Índice** | Base de datos donde buscar | **Azure AI Search**: vectores, palabras clave y filtros en un solo servicio | **Vector Search / Agent Retrieval**, **RAG Engine** (base de datos gestionada, Pinecone o Weaviate) o **Agent Search** (totalmente gestionado) | **Qdrant, Chroma**, Weaviate, Milvus, pgvector, Elasticsearch / OpenSearch † |
| **6. Permisos** | Cada usuario ve solo su contenido | Entra ID + filtro por grupo en el índice | Control de acceso de Google (IAM) + control de acceso por fuente de datos en Agent Search | Filtros por metadatos en la base de datos vectorial † |

#### Fase 2: Responder una pregunta

| Etapa | Qué hace | Azure | Google Cloud | Código abierto |
|----|----|----|----|----|
| **7. Reescritura** | Pregunta autónoma; dividir preguntas complejas | Query rewriting del semantic ranker (preview); búsqueda agéntica con planificación (preview) | Agent Search: preguntas de seguimiento y respuestas con búsqueda agéntica | LangChain MultiQueryRetriever, HyDE, transformaciones de consultas de LlamaIndex † |
| **8. Palabras clave (BM25)** | Coincidencia exacta | BM25 integrado en AI Search | **Vector Search**: tú generas el vector sparse (BM25, TF-IDF o SPLADE) y lo subes. **Agent Search**: gestionado | BM25 de Elasticsearch/OpenSearch; vectores sparse en Qdrant; SPLADE |
...
| **11. Reranker** | Reordenar leyendo pregunta y chunk juntos | **Semantic ranker**: el top 50, puntuación 0–4 | **Ranking API**: `semantic-ranker-default-004` / `-fast-004` (1024 tokens, 25 idiomas, puntuación 0–1, hasta 1000 chunks por llamada). La versión 005 está en preview desde el 1 de septiembre de 2026 y pasará a ser la predeterminada **a más tardar el 1 de octubre de 2026** | **bge-reranker-v2-m3**, **Qwen3-Reranker** (Apache 2.0), **mxbai-rerank-v2** (Apache 2.0), ColBERTv2; o un modelo de lenguaje como reranker |
...
| **13. GraphRAG** | Grafo para preguntas globales | Microsoft **GraphRAG** (código abierto) desplegado en Azure; LazyGraphRAG en Microsoft Discovery | No se encontró un equivalente gestionado (al 2026-09-23) | **GraphRAG** (Microsoft), **LightRAG**, **HippoRAG 2** |

#### Fase 3: Generación y guardrails

| Etapa | Qué hace | Azure | Google Cloud | Código abierto |
|----|----|----|----|----|
| **14. Modelo redactor** | Respuesta con citas | Modelos GPT en Azure OpenAI / Microsoft Foundry (también otros modelos en Foundry †) | **Gemini** (familia 3.x); también Claude, Llama, Qwen y otros en Model Garden | Llama, Qwen, Mistral, gpt-oss servidos con **vLLM** u **Ollama** † |
| **15. Protección de entrada** | Bloquear intentos de manipulación | Content Safety – Prompt Shields † | **Model Armor** | NeMo Guardrails, Llama Guard † |
| **16. Fidelidad a los documentos** | ¿Cada oración está respaldada? | Evaluador Groundedness; Groundedness Pro (preview) | **Check Grounding API**: puntuación 0–1 por afirmación + citas, en menos de 500 ms | **HHEM-2.1-Open** (Vectara), MiniCheck |

#### Fase 4: Evaluación y monitoreo

| Etapa | Qué hace | Azure | Google Cloud | Código abierto |
|----|----|----|----|----|
| **17. Evaluar el retrieval** | ¿Encontró lo correcto? | **Document Retrieval** (NDCG, XDCG, Fidelity, Holes; necesita etiquetas) y **Retrieval** (juez, sin etiquetas) | **Evaluate search quality** en Agent Search; servicio de evaluación de Agent Platform | **RAGAS**, **DeepEval**, **RAGChecker**, **Open RAG Eval** (UMBRELA) |
...
| **19. Evaluación continua** | Evaluar muestras del tráfico real | **Evaluación continua** de Foundry: muestreo configurable, hasta 1000/hora, resultados en Application Insights | **Online Monitors**: cada ~10 min, porcentaje y tope configurables, resultados en Cloud Logging y Cloud Monitoring | Langfuse †, **Arize Phoenix**, **MLflow** |
...
|----|----|----|----|
| Leer docs | Document Layout skill | Document AI Layout Parser | Docling / Unstructured |
...
1.  **Búsqueda híbrida.** En Azure AI Search, la búsqueda por palabras clave viene incluida. En **Google Vector Search tienes que generar tú mismo el vector sparse**; si quieres que Google lo gestione, usa Agent Search. En código abierto depende de la base de datos.
2.  **Reranker.** El de Azure reordena solo el **top 50**. La Ranking API de Google acepta **hasta 1000 chunks** y funciona con cualquier buscador, incluso uno externo. En código abierto controlas el modelo, el costo y la latencia, pero también tienes que operarlo.
3.  **Evaluación.** Ambas nubes ya ofrecen evaluación offline y evaluación continua sobre tráfico real, con trazas estándar (OpenTelemetry). En código abierto, RAGAS o DeepEval (offline) más Phoenix, MLflow o Langfuse (producción) cubren lo mismo, pero la integración la haces tú.

## Apéndice · Glosario

| Término | Significado en simple |
|----|----|
| **Búsqueda agéntica** | Un agente divide la pregunta y ejecuta varias búsquedas |
| **BM25** | Algoritmo clásico de búsqueda por palabras clave |
| **Chunk** | Un trozo de documento que se indexa por separado |
| **Kappa de Cohen** | Medida de acuerdo entre dos evaluadores que descuenta el acuerdo por azar |
| **Completeness** | Que la respuesta no deje fuera información importante |
| **Integración continua (CI)** | Pruebas automáticas que se ejecutan en cada cambio de código |
| **Cross-encoder / bi-encoder** | Lee ambos textos juntos / convierte cada texto en un vector por separado |
| **Denso / sparse** | Un vector con todos sus valores activos / un vector de términos con pesos, casi todos en cero |
...
| **Búsqueda híbrida** | Combinar la búsqueda por palabras clave con la búsqueda por significado |
...
| **Modelo de lenguaje (LLM)** | El modelo que redacta la respuesta (GPT, Gemini, Claude, Llama…) |
| **Juez LLM** | Otro modelo que puntúa las respuestas |
| **MMR** | Una técnica para eliminar resultados redundantes |
| **MRR** | Qué tan arriba aparece el primer resultado correcto |
| **nDCG** | Calidad del orden: si los correctos están lo más arriba posible (los evaluadores de Azure lo escriben *NDCG*) |
...
| **Precision@k** | Qué fracción de los top k resultados es correcta |
...
| **Recall@k** | Qué fracción de los correctos aparece en los top k resultados |
| **Reranker** | Un modelo que reordena los candidatos leyendo la pregunta y el chunk juntos |
| **RRF** | Fusión por rango recíproco: combinar listas usando solo las posiciones |
| **SPLADE / ELSER** | Modelos que expanden el texto con términos relacionados (sparse aprendido) |
...
- RRF en búsqueda híbrida: <https://learn.microsoft.com/en-us/azure/search/hybrid-search-ranking>
...
- Anthropic, *Contextual Retrieval*, 2024 — <https://www.anthropic.com/engineering/contextual-retrieval> (cifras verificadas a través de un mirror; la página original bloqueó el acceso automatizado)
...
RAG de punta a punta: un tutorial práctico · compilado el 2026-09-23. Los datos marcados con † no se volvieron a verificar contra la documentación oficial en esa fecha; las funciones marcadas con (preview) no están disponibles de forma general.
